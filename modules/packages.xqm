...
declare function packages:get-local-packages(){
    let $apps := packages:default-apps() | packages:installed-apps()

    let $auth := packages:get-user-access-level()
    return
        if (xs:integer($config:VIEW-PACKAGE-PERMISSION) >= 10) then (
            for $app in $apps
            order by upper-case($app/title/text())
            return
                packages:display($config:REPO, $app, $auth)
        ) else (
...
declare %private function packages:display($repoURL as xs:anyURI?, $app as element(app), $auth as xs:integer) {
    let $icon :=
        if ($app/icon) then
            if ($app/@status) then
                $app/icon[1]
...
(:
declare %private function packages:display-legacy($repoURL as xs:anyURI?, $app as element(app), $format as xs:string?) {
    let $icon :=
        if ($app/icon) then
            if ($app/@status) then
                $app/icon[1]
